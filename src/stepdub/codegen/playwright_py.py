...
1. The generated file has to be maintainable by hand. The test is simple: can someone
   delete stepdub from the machine and keep editing that Python? If not, the generator
...
def _plain(text: str) -> str:
    """Recorded text made safe for a comment: one line, printable characters only."""
    return "".join(ch if ch.isprintable() else " " for ch in text)


def _doc(text: str) -> str:
    """Recorded text made safe inside a triple-quoted docstring."""
    return _plain(text).replace("\\", "\\\\").replace('"', '\\"')


def _check_name(name: str, what: str) -> None:
    """A name the user chose, about to become an identifier in the generated code."""
    if not name.isidentifier() or keyword.iskeyword(name):
        raise ValueError(f"{what} {name!r} is not a valid Python name")
    if name in _RESERVED:
        raise ValueError(f"{what} {name!r} is already used by the generated code")
...
class _Generator:
    def __init__(self, rec: Recording, options: GenOptions) -> None:
        _check_name(options.function_name, "function name")
        for name in options.params.values():
            _check_name(name, "parameter name")
    ...
    def _base(self, target: Target, page: int, em: _Emitter, depth: int) -> str:
        ...
        if var is None:
            ...
            message = _lit(f"frame not found: {target.frame_url}")
            em.line(f"raise RuntimeError({message})", depth + 1)
        return var
    ...
    def _emit_action(self, ev: Event, em: _Emitter, depth: int, nxt: Event | None) -> None:
        ...
        if key not in self.commented:
            ...
            if comment:
                em.line(f"# {_plain(comment)}", depth)
        ...
        if ev.action is Action.WAIT:
            ...
            if ev.is_secret:
                ref = _plain(ev.secret_ref or "")
                em.line(f"# value comes from {ref}, it was never recorded", depth)
    ...
    def _emit_header(self, em: _Emitter) -> None:
        name = _doc(self.rec.meta.name or self.rec.meta.slug)
        em.line('"""' + f"{name} - generated by stepdub from a recording.")
        em.line()
        em.line(f"Regenerate:  stepdub gen {_doc(self.rec.meta.slug)}")
        em.line("Edit by hand: go ahead. stepdub never reads this file back.")
        if self.secrets:
            em.line()
    ...
    def _emit_run(self, em: _Emitter) -> None:
        ...
        name = _doc(self.rec.meta.name or self.rec.meta.slug)
        em.line('"""' + f"Replay the steps recorded in '{name}'." + '"""', 1)
        if self.has_downloads:
